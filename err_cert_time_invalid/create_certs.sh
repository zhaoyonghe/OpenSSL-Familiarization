...
###################################################################################
# server certificate
cp www.test.com.cnf ${INTERMEDIATE_CA_DIR}/www.test.com.cnf
new_step "Generate key pair for the server"
openssl genrsa -aes256 -passout pass:yyyy -out ${INTERMEDIATE_CA_DIR}/private/www.test.com.key.pem 2048
chmod 400 ${INTERMEDIATE_CA_DIR}/private/www.test.com.key.pem
new_step "Generate a server CSR"
openssl req -config ${INTERMEDIATE_CA_DIR}/www.test.com.cnf \
    -new -sha256 -passin pass:yyyy \
...
openssl ca -config ${INTERMEDIATE_CA_DIR}/intermediate_ca.cnf -extensions server_cert \
    -startdate 20150101120000Z -enddate 20160101120000Z -notext -md sha256 -passin pass:yyyy \
    -in ${INTERMEDIATE_CA_DIR}/csr/www.test.com.csr.pem \
